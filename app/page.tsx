...
const FUNCTIONAL = new Set(["new-chat", "chats"])
const WINDOW_TOOLS = new Set(["settings", "usage"])

function GaiaApp() {
  const { settings } = useGaia()
  ...
  return (
    <div className="flex h-dvh w-full overflow-hidden bg-background text-foreground">
      ...
      {/* Mobile drawer */}
      <div className={cn("fixed inset-0 z-[100] lg:hidden", mobileOpen ? "pointer-events-auto" : "pointer-events-none")} aria-hidden={!mobileOpen}>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border px-4 sm:px-6">
          ...
          <PanelLeft className="hidden size-[18px] text-muted-foreground lg:block" aria-hidden="true" />
          <div className="flex flex-col leading-tight">
            <h1 className="text-sm font-semibold capitalize text-foreground">
              {active === "new-chat" ? "New Chat" : active.replace("-", " ")}
            </h1>
            <span className="text-xs text-muted-foreground">
              {settings.assistantName} · {settings.model.charAt(0).toUpperCase() + settings.model.slice(1)}
            </span>
          </div>
        ...
        <div className="relative min-h-0 flex-1">
          ...
          {minimized.length > 0 && (
            <div className="pointer-events-none absolute inset-x-0 bottom-3 z-[90] flex justify-center px-4">
              <div className="pointer-events-auto flex max-w-full items-center gap-2 overflow-x-auto rounded-2xl border border-border bg-card/90 p-1.5 shadow-xl shadow-black/30 backdrop-blur">
                {minimized.map((w) => {
                  ...
                  return (
                    <div key={w.id} className="flex shrink-0 items-center">
                      <button type="button" onClick={() => openWindow(w.id)}
                        className="flex min-h-9 items-center gap-2 rounded-l-xl rounded-r-none bg-secondary/70 px-3 text-xs font-medium text-foreground transition-colors hover:bg-secondary">
                        <Icon className="size-4 text-primary" aria-hidden="true" />
                        {meta.label}
                      </button>
                      <button type="button" onClick={() => closeWindow(w.id)} aria-label={`Close ${meta.label}`}
                        className="flex min-h-9 items-center rounded-l-none rounded-r-xl bg-secondary/70 pl-1 pr-2 text-muted-foreground transition-colors hover:bg-destructive/20 hover:text-destructive">
